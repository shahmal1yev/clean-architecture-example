...
namespace Tests\Unit\Presentation\DTO\BookCreatedDTO;
...
class SerializationTest extends TestCase
{
    ...
    {
        $dto = new BookCreatedDTO($this->mockBook());
        $actual = $transformer($dto);

        foreach($this->expectedDataStructure() as $fieldName => $dataType) {
            $this->assertSame($dataType, gettype($actual[$fieldName]));
        }
    }

    public static function transformerProvider(): \Generator
    {
        yield [fn(BookCreatedDTO $dto): array => json_decode(json_encode($dto), true)];
        yield [fn(BookCreatedDTO $dto): array => $dto->jsonSerialize()];
    }
